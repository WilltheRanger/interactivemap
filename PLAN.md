...
- **[12 — Hall-pass Log (QR → teacher's Google Sheet, zero-submit)](plan/phase-12-hall-pass-log.md)** — Tap **Log** → pick a reason → scan the teacher's QR → the teacher's **Apps Script** appends `{time, student, reason}` to their own Sheet (no submit); the app stores nothing, so the privacy rule holds. Chunks: URL-safety + reason builder (built) · Reason picker · QR scanner · Log entry point · Teacher setup guide (built) · Privacy assertion. *Complete when:* a valid teacher QR logs a row, disallowed QRs are rejected, and an audit shows the app retains no log data.
