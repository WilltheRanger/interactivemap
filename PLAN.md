...
1. **Schedule view: passive.** Tap a period → see its class/room/teacher. **No** live now/next auto-tracking. (Optional static "current period" label is pending confirm; off by default.)
...
| E | **Bell schedule** (period times, day types) — **optional** (only if showing period time ranges / the optional current-period label) | pending | schedule | 07 |
...
- **[07 — Schedule display & map highlight (passive)](plan/phase-07-schedule-display-and-highlight.md)** 🚩 — Show the schedule, tap a period → its class+location, and highlight the student's classes on the map. No live now/next. Chunks: Schedule list · Tap-to-locate · Static map highlight · (optional) current-period label · Sync. *Complete when:* the schedule renders, tapping a period highlights its building, and all class buildings highlight on the map.
